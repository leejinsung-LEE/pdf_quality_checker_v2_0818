...
# 새로운 설정 뷰 (선택적 사용)
try:
    from ...views.settings_v2 import SettingsViewV2
...
class DialogManager:
    ...
    def __init__(self, window: 'MainWindow'):
        self.window = window
        # 새 UI 사용 여부 (설정에서 읽거나 기본값 사용)
        self.use_new_ui = self._check_new_ui_preference()
    
    def show_preferences(self):
        """환경설정 다이얼로그"""
        # 새 UI 사용 여부에 따라 다른 뷰 사용
        if self.use_new_ui and SETTINGS_V2_AVAILABLE:
            self._show_preferences_v2()
        else:
            self._show_preferences_legacy()
    
    def _show_preferences_legacy(self):
        """기존 환경설정 다이얼로그"""
    ...
    def _show_preferences_v2(self):
        """새로운 환경설정 다이얼로그 V2"""
        # SettingsViewV2는 이미 Toplevel을 상속받음
        settings_view = SettingsViewV2(self.window)
        
        # V2는 즉시 적용 모드이므로 별도의 콜백이 필요없음
        # 대신 닫힐 때 UI 업데이트
        def on_close():
            # 테마 업데이트
            current_settings = self.window.settings_controller.get_settings()
            if hasattr(current_settings, 'theme'):
                ctk.set_appearance_mode(current_settings.theme)
            
            # 사이드바 너비 업데이트
            if hasattr(self.window, 'sidebar') and hasattr(current_settings, 'sidebar_width'):
                self.window.sidebar.configure(width=current_settings.sidebar_width)
        
        settings_view.on_close = on_close
    ...
    def _check_new_ui_preference(self) -> bool:
        """새 UI 사용 여부 확인"""
        try:
            # 설정에서 확인
    ...
    def toggle_ui_version(self):
        """UI 버전 전환"""
        self.use_new_ui = not self.use_new_ui
        
        # 설정에 저장
        try:
            self.window.settings_controller.update_setting('use_new_ui', self.use_new_ui)
        except:
            pass
        
        version = "V2 (새 버전)" if self.use_new_ui else "V1 (기존 버전)"
        messagebox.showinfo("UI 버전 전환", f"환경설정 UI가 {version}로 전환되었습니다.\n다음 번 환경설정을 열 때 적용됩니다.")